//컨텐츠 페이지의 #user 입력된 값이 변경됐을 때
document.querySelector('#user').addEventListener.istener('change', function(){var user = document.querySelector('#user').value;

//컨텐츠 페이지에 몇 개의 단어가 등장하는지 계산해주세요.

//컨텐트 페이지의 모든 텍스트를 가져온다. 그 결과를 bodyText 변수에 답는다.
//크롬 확장의 기능 중에 tabs와 관련된 기능 중에 컨텐츠 페이지를 대상으로 코드를 실행해주세요.

chrome.tabs.executeScript({
    code:'document.querySelector("body").innerText'
}, function(comes){
    //위 코드가 실행된 후에 이 함수를 호출해주세요. 그 때 result에 담아주세요.
...
function show_more()
{
    window.open('https://google.co.kr');
}
